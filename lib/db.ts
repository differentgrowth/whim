...
export const checkCustomerExists = async ({ email }: { email: string }) => {
  return prisma.customer.findUnique({ where: { email }, select: { id: true } });
};

export const createWhim = async ({
  customerId,
  url,
  name,
  expiration,
  ...
  return prisma.url.create({
    data: {
      ...
      expiration,
      password,
      secret_key: password ? createSecretKey() : null,
      Customer: {
        connect: {
          id: customerId,
        },
...
export const getCustomerWhims = async ({
  customer_id,
}: {
  customer_id: string;
}) => {
...
export const increaseWhimCounter = async ({
  shorted_url,
}: {
  shorted_url: string;
}) => {
...
export const getWhimPassword = async ({
  shorted_url,
}: {
  shorted_url: string;
}) => {
