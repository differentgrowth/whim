...
export const createCustomer = async ( { email }: { email: string; } ) => {
  return prisma.customer.create( { data: { email } } );
};
...
export const getWhim = async ( { shorted_url }: { shorted_url: string } ) => {
  return prisma.url.findFirst( { where: { shorted_url }, select: { url: true } } );
};

export const getCustomerWhims = async ( { customer_id }: { customer_id: string } ) => {
  return prisma.url.findMany( { where: { customer_id }, orderBy: { created_at: 'desc' } } );
};
...
export const increaseWhimCounter = async ( { shorted_url }: { shorted_url: string } ) => {
  return prisma.url.update( { where: { shorted_url }, data: { counter: { increment: 1 } } } );
};